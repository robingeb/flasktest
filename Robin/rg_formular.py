...
from rg_forms import NameForm, articleForm, choiceform, erpformweclapp, erpformdynamics, erpformmyfactory, erpformxentral, articlehomeForm
import requests
...
def signup(Artikelnr):
    ...
    if form.validate_on_submit():
        ...
        datasets = col.insert_one({"Mängel": str(mängel), "name": str(name), "accept": accept, "Datum": timestamp2, "nächstes Prüfdatum": timestamp, "Artikelnummer": str(Artikelnr)})
